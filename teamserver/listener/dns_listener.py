...
class DnsListener(BaseListener):
    ...
    def start(self):
        print(f"[*] Starting DNS listener {self.config.get('name', 'dns_listener')} on {self.host}:{self.port}")
        self._running.set()
        self.thread.start()
    ...
    def _run(self):
        ...
        print(f"[+] DNS Listener {self.config.get('name', 'dns_listener')} started on port {self.port}.")
        ...
        sock.close()
        print(f"[*] DNS Listener {self.config.get('name', 'dns_listener')} stopped.")
        ...
        try:
            ...
            if op == "result" and len(parts) >= 4:
                ...
                try:
                    ...
                    print("[TEAMSERVER][ERROR] Can't decode result:", e)
                reply = req.reply()
            ...
            print(f"[!] DNS request parse error from {addr}: {e}")

    def stop(self):
        print(f"[*] Stopping DNS listener {self.config.get('name', 'dns_listener')}")
        self._running.clear()
